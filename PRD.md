...
Sleeves need the arm's axis and trousers need each leg's; a ring has neither. Producing them means a limb-aware garment builder driven by the rig — which is not a small pipeline addition, it is writing a garment modeller. **So the fallback v1.7 framed for MakeHuman is now the live path: a second CC0 garment source, authored with real garment topology.** Which source is the owner's call, and it is section 9's asset-pipeline question reopening for wardrobe specifically — v1.7 closed it for *bodies* only. **Until that is chosen, nothing under "Authorized to build" below can proceed except the hair, which the spike did land** (a fitted ellipsoid cap cut back from the face, plus a gathered mass — it reads at 35 mm and it is silhouette-only, exactly what this amendment admits). The wardrobe half of v1.10 is blocked on an asset decision, not on engineering.
...
1. **Hair derivation in `scripts/blender/build-pose-glbs.py`** — a fitted ellipsoid cap cut back from the face, plus a gathered mass, applied before the armature is applied and the rig deleted, so it poses with the body. **Garment derivation is struck**, per the spike above; wardrobe geometry waits on the source decision.
