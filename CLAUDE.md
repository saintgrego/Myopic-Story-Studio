...
- **Tests** live in `src/__tests__/` and run on CRA's bundled Jest 27 (`npm test` for watch mode, `npm run test:ci` for one shot). They cover the `.myo` envelope mapping, the parser's flag/poseNote post-processing (Anthropic API mocked via `global.fetch`), the `sceneStore` `setField` path machinery, and poses.json / props.json ↔ `public/assets/*/**.glb` consistency (both directions for props — an unregistered `.glb` is invisible to the panel and parser). `poses.test.ts` also loads each pose `.glb` and asserts `Box3.min.y ≈ 0` (the base-anchored convention); it does **not** detect a stale binary, only an ungrounded one. Three gates now: `npx tsc --noEmit`, `npm run test:ci`, `npm run build`. Note `npx jest` directly does **not** work — the Jest config lives inside `react-scripts`.
  ...
  - `src/setupTests.ts` backfills `structuredClone` (Jest 27's jsdom predates it) — sceneStore tests break without it — and `TextDecoder`, which `GLTFLoader` needs.
  - Loading a `.glb` in a test: `package.json`'s `jest` block carves `three/examples/jsm/` out of `transformIgnorePatterns` (untransformed ESM otherwise) and maps `^three$` to `build/three.cjs` (Jest 27 ignores the `exports` field and would otherwise pull the deprecated UMD build). **CRA jest overrides replace arrays and merge objects** — the CSS-module pattern is copied into `transformIgnorePatterns` by hand for that reason. Pass the loader a realm-local buffer (`Uint8Array.from(buf).buffer`): a Node `fs` ArrayBuffer fails GLTFLoader's `instanceof ArrayBuffer` check inside jsdom and surfaces as a bogus "Unsupported asset" error. See STATE.md 2026-08-10.
...
**A pose is a mesh, not a field** (PRD §11 v1.2): `/assets/poses/sitting.glb` *is* the sitting pose. There is no `pose` field anywhere. `src/poses.json` is the single source of truth for the pose library — `PropertiesPanel` imports it, `server/parser.js` `require`s it. Adding a pose = add a row to a generator's `POSES` table, re-run it, add a row to `poses.json`. No viewport code exists to touch. **The library is now authored figure geometry, in two builds** (PRD §11 v1.7, second figure in v1.8 — the bare names are the *default* figure and the `-female` suffix the other; the bare paths are kept because saved `.myo` files reference them): `scripts/blender/build-pose-glbs.py` poses a CC0 Blender Studio base mesh in headless Blender and is the real generator; `scripts/generate-pose-glbs.mjs` survives as the no-Blender fallback and *overwrites the good figures with primitives* if you run it by reflex. Both tables must stay in step. **v1.10 adds two more tables to the same file**: `HAIR` (suffix → `bare`/`cropped`/`gathered`, derived geometry, bone-parented to the head) and `GARMENT_FIGURES` (suffix → body + hand-authored garment objects loaded from `assets-src/garments.blend`, shipped empty until garments are modelled — deriving garments from the body was tried and failed, see STATE.md). `bake_and_export()` takes a **list** of meshes and grounds them as one group; the committed `.glb`s predate the hair table, so they are bald until someone re-runs `build:poses`; that run no longer needs the 48 MB CC0 bundle — `<source>` takes either the bundle `.blend` or a directory of already-exported `standing<suffix>.glb` (`load_standing_glb`) — though **garments** still do, being authored in the bundle's coordinate space. The parser's unmatched-posture flag (`characters[i].poseNote` in `flaggedParams`) is computed then **stripped** before the scene is built — `poseNote` never reaches the scene model or disk.
...
**The rig (PRD v2.0 phase 1) exists; the pose model does not yet.** `public/assets/figures/mannequin{,-female}.glb` are skinned rest-pose figures with the Mixamo-named joint set in `src/rig.ts` (`JOINT_PARENT` — 20 joints; the PRD's "19" miscounts its own list). `scripts/blender/build-figure-glbs.py` builds them and imports measurement/binding from `build-pose-glbs.py` (whose `main()` is now `__main__`-guarded — keep it so). `Viewport.tsx` strips `mixamorig` prefixes from every loaded glTF; GLTFLoader has already deleted the colon by then (`mixamorigHips`). Axis convention and the "weld before bone heat" gotcha: STATE.md "Rig: phase 1". Nothing references the figures until phase 2, so "a pose is a mesh" above still describes the running app.
...
- **Deleting a remote branch cannot be done from a Claude Code web/remote session** — the agent git proxy rejects ref deletion with `RPC failed; HTTP 403` for every branch, while ordinary pushes and fetches on the same remote work fine. `git push` then exits `0` and prints "Everything up-to-date", so the failure is easy to miss; confirm with `git ls-remote --heads origin`. Retrying, `--force`, and the `:refs/heads/<branch>` refspec all fail identically. Don't debug it — say it needs doing on GitHub (the **Delete branch** button on the merged PR, or the same command from a local checkout), and move on. Deleting the *local* branch works normally.
- **Keep the session title current (Claude Code web/remote sessions).** The owner finds where they left off by scanning session titles, so title every session `Myopic · <topic> · <STATUS> → next: <next step>`, where STATUS is one of `IN PROGRESS`, `WAITING ON YOU`, `DONE`, `BLOCKED`. Example: `Myopic · Articulation: rig · IN PROGRESS → next: bone naming test`. Set it once the task is clear, and update it whenever the status or next step changes — at minimum when work pauses for the owner and when it finishes. Use the `set_session_title` tool (claude-code-remote MCP server; `get_session` with no id returns this session's id). Keep "next" to one concrete step.
